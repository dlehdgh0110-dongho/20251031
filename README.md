...
- `generate_efficient_frontier.py`는 종가 기반의 일간 수익률을 계산한 후, 연환산 기대수익률과 변동성을 도출합니다. 장기 보유 포트폴리오 가정을 위해 거래일 252일을 기준으로 연환산했습니다.

```bash
python generate_efficient_frontier.py
```
- 3자산(005930.KS, AAPL, NVDA)에 대해 1/60 간격으로 장기 보유(long-only) 가중치를 그리드 탐색하여 전체 포트폴리오 조합을 만들고, 위험 대비 수익이 우수한 조합만 남기는 방식으로 효율적 프론티어를 구성했습니다.
- 아래 도표는 연환산 변동성(가로축)과 기대수익률(세로축)의 관계를 나타냅니다.

![Efficient frontier for the three-asset universe](efficient_frontier.svg)

### 핵심 시사점
- **최소 분산 포트폴리오**는 삼성전자 43.3%, Apple 51.7%, NVIDIA 5.0% 비중으로 구성되며 연환산 변동성 21.7%, 기대수익률 30.2% 수준입니다.
- **최대 기대수익 포트폴리오**는 NVIDIA 100% 단일 종목 포지션으로, 연환산 변동성 49.4%, 기대수익률 94.4%에 해당합니다.
- 효율적 프론티어 상에서 변동성을 소폭 늘리면 기대수익이 급격히 증가하는 구간이 존재해, 고위험/고수익 성향 투자자의 경우 NVIDIA 비중 확대가 유리하지만, 변동성 관리 측면에서는 3자산 혼합이 더 안정적인 선택임을 보여줍니다.
